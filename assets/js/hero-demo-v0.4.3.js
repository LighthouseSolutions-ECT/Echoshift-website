...
if (demo) {
  ...
  const updateToggle = () => {
    if (!toggle) return;
    const label = hasCompleted ? 'Replay' : isPaused ? 'Play' : 'Pause';
  ...
  const restart = () => {
    ...
    demo.classList.remove('is-paused', 'is-complete', 'is-announcing', 'is-content-refreshing');
  ...
  document.addEventListener('visibilitychange', () => {
    if (document.hidden && !hasCompleted && !isPaused) {
      visibilitySuspended = true;
      pauseTimeline();
    } else if (!document.hidden && visibilitySuspended && !isPaused && !hasCompleted) {
      visibilitySuspended = false;
